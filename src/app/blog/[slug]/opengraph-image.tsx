...
import { fetchBlogPostServer } from '@/hooks/useBlog';

/**
 * Per-post OG card, generated at build time. Next.js invokes this
 * function for each known slug and writes a static PNG into the build
 * output; the sibling page's <meta property="og:image"> auto-points
 * at the result.
 *
 * If the post can't be fetched at build (e.g. API hiccup), falls back
 * to a generic EchoDesk card — better a valid image than a 404.
 */

export const alt = 'EchoDesk blog post';
...
export default async function BlogPostOGImage({
  ...
  // Fetch the post to get its title. Prefer Georgian (our primary market),
  // fall back to English if that locale is empty.
  let title = 'EchoDesk';
  let tag = 'BLOG';
  try {
    const post = await fetchBlogPostServer(params.slug, 'ka');
    if (post?.title) title = post.title;
    if (post?.post_type) tag = POST_TAG[post.post_type] ?? 'BLOG';
  } catch {
  ...
  // Auto-shrink title font as length grows so long comparisons still fit.
