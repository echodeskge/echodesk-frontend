...
export default function TermsOfService() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      setLoading(true);
      try {
        const filename = language === "ka" ? "terms-georgian.md" : "terms-english.md";
        const response = await fetch(`/${filename}`);
        const text = await response.text();
        setContent(text);
      } catch (error) {
        console.error("Error fetching terms of service:", error);
        setContent("Error loading terms of service content.");
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="prose prose-gray max-w-none">
            <ReactMarkdown
              components={{
                ...
                ol: ({ children }) => (
                  <ol className="list-decimal pl-6 space-y-2 mb-4 text-gray-700">{children}</ol>
                ),
                li: ({ children }) => (
