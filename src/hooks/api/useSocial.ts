...
// ============================================================================
// SOCIAL SYNC HOOKS (Facebook & Instagram Message History)
// ============================================================================

export type SyncStatusType = 'pending' | 'syncing' | 'completed' | 'failed';

export interface FacebookPageSyncStatus {
  page_id: string;
  page_name: string;
...
export interface InstagramAccountSyncStatus {
  account_id: string;
  username: string;
  sync_status: SyncStatusType;
  last_sync_at: string | null;
...
export interface SocialSyncStatusResponse {
  facebook?: {
    pages: FacebookPageSyncStatus[];
    total_pages: number;
  ...
  instagram?: {
    accounts: InstagramAccountSyncStatus[];
...
export const syncKeys = {
  all: ['social', 'sync'] as const,
  status: (platform?: string) => [...syncKeys.all, 'status', platform] as const,
};

/**
 * Get sync status for Facebook pages and/or Instagram accounts
 */
export function useSocialSyncStatus(platform?: 'facebook' | 'instagram') {
  return useQuery<SocialSyncStatusResponse>({
    queryKey: syncKeys.status(platform),
    queryFn: async () => {
      const url = platform
        ? `/api/social/sync/status/${platform}/`
        : '/api/social/sync/status/';
      const response = await axios.get(url);
      return response.data;
    },
    staleTime: 30 * 1000, // Consider data fresh for 30 seconds
    refetchInterval: 10 * 1000, // Poll every 10 seconds to show sync progress
  });
}
 ...
 */
export function useTriggerSocialSync() {
  const queryClient = useQueryClient();
  ...
  return useMutation<SyncTriggerResponse, Error, {
    ...
    onSuccess: () => {
      ...
      // Also invalidate message queries since new messages may have been synced
      queryClient.invalidateQueries({ queryKey: socialKeys.facebookMessages() });
      queryClient.invalidateQueries({ queryKey: socialKeys.instagramMessages() });
    },
  });
...
export function useUpdateSyncSettings() {
  ...
  return useMutation<
    { platform: string; account_id: string; account_name: string; sync_days_back: number },
    Error,
    { platform: 'facebook' | 'instagram'; account_id: string; sync_days_back: number }
  >({
    mutationFn: async (data) => {
      const response = await axios.post('/api/social/sync/settings/', data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: syncKeys.all });
